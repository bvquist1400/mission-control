...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center rounded-card border border-dashed border-stroke bg-panel py-16 text-center">
      ...
      <p className="mt-1 text-sm text-muted-foreground">Create your first implementation using the form above.</p>
    </div>
  );
}
...
export default function ImplementationsPage() {
  ...
  async function createImplementation(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!name) {
      setError("Implementation name is required");
      return;
    }
    ...
    try {
      ...
      setError(createError instanceof Error ? createError.message : "Failed to create implementation");
    } finally {
      setIsCreating(false);
    }
  }
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-card border border-stroke bg-panel p-4 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h2 className="text-sm font-semibold text-foreground">Add Implementation</h2>
