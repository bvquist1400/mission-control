-- Transaction-safe template authoring RPC (create + replace-update).
...
BEGIN
  ...
  ELSE
    ...
    SET
      ...
      default_stage = COALESCE(p_default_stage, default_stage),
      default_rag = COALESCE(p_default_rag, default_rag),
    ...
    DELETE FROM project_template_sections
    WHERE template_id = v_template_id
      AND user_id = p_user_id;
  END IF;
  ...
  LOOP
    ...
    IF task_entry ? 'checklist_items' AND jsonb_typeof(task_entry->'checklist_items') = 'array' THEN
      ...
      LOOP
        ...
        SET checklist_items = checklist_items || v_checklist_text
        WHERE id = v_new_task_id
          AND user_id = p_user_id;

        v_checklist_item_count := v_checklist_item_count + 1;
        v_checklist_sort_order := v_checklist_sort_order + 1;
      END LOOP;
    END IF;
  END LOOP;
